...
+ **总结：** 
  ...
  + **JavaScript是单线程的原因？** 
    ...
    > **1. 避免复杂性，提高效率。**  
    >
    > **2. 和用途有关，如多个线程去操作一个DOM的话（比如一个线程添加内容，一个线程删除元素），浏览器难以控制。** 
...
+  **总结** 
  ...
  + **任务队列** 
    ...
    > 在任务对象中的任务，是必须要等前一个任务执行完毕后，后续的任务才能够执行。
  ...
  + **运行机制** 
    ...
    > 1. 所有的同步任务在主线程上，形成要给**执行栈** 
    > 2. 在主线程之外，有个任务队列，当异步任务有了执行结果，就会在“任务队列”中放置一个事件
    > 3. 当主线程上的执行栈全部结束后，检测任务队列中是否存在异步任务，若存在，则结束等待状态，放置到执行栈中，执行。
    > 4. 重复第3步
...
1. 主线程执行同步任务，
2. 宿主环境执行异步任务，并将结果以回调的方式放置在任务队列中
3. 主线程的同步任务执行完毕后，会检测任务队列中的回调是否有任务通知
4. 若有，则取出任务放置主线程中执行。
5. 重复第三步和第四步。